...
def process_ticket(
    ...
    *,
    ...
    def _success_update(parsed_output):
        ...
        return {"parsed_json": parsed_json, "mechanism": mechanism}
...
def main(
    *,
    ...
    for idx, (ticket_id, canonical_failure, thread_context) in enumerate(rows, 1):
        ...
        _log(f"[pass2]   canonical_failure: {canonical_failure[:80]}{'…' if len(canonical_failure) > 80 else ''}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pass 2 — Failure mechanism inference from Pass 1 canonical failure."
    )
    ...
    main(ticket_ids=ticket_ids, limit=args.limit, force=args.force, failed_only=args.failed_only)
