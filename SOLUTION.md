...
web/app.py             — Dash + Mantine web dashboard entry point (read-only, live Postgres queries; run with `python -m web.app`); includes manual dark mode toggle in the header (sun/moon switch with localStorage persistence via clientside callback that toggles `body.dark-mode` CSS class)
...
web/pages/health.py    — Health dashboards: Customer and Product health AG Grid tables; Customer Health now shows one row per active TeamSupport customer, includes `Key Account`, computes default distress across all groups except Marketing/Sales, and supports multi-row "View Tickets" drill-down plus single-customer history modal with group selector, Matcha-backed Explain button, and persisted explanation history; Customer Health grid columns use flex sizing with minWidth so all columns fill the available viewport width; column order places Distress and Band immediately after Customer (before Key Acct) for quick triage visibility; grid now surfaces all five distress sub-dimensions (Pressure, Aging, Friction, Concentration, Breadth) matching the Health History dialog breakdown, each with color-coded highlights when above threshold (custom)
web/pages/operations.py — Operations dashboard: analyst behaviour analytics for detecting gaming patterns. Analyst Scorecard AG Grid (tickets closed, avg days open, high sev %, technical %, scheduling %, high priority/frustrated counts, own-work ratio, avg other contributors, zero-contribution closes, % low-contribution closes) with red highlights for low technical/high-severity avoidance and row-click drill-down into low-contribution tickets. Technical vs Scheduling Work chart (horizontal grouped bar of technical work % and scheduling % per analyst, red for ≥ 20%% below tech avg, orange for ≥ 30%% above scheduling avg). High-Severity Closure Share chart (horizontal bar of high-sev % per analyst, red for ≥ 35%% below team avg). Free-Rider Detector chart (horizontal bar of own-work ratio, red < 50%%, orange < 70%%). Monthly Closures by Analyst line chart (12 months, top 10). Swooper drill-down modal shows tickets where the analyst closed but contributed < 25%% of InHance actions (custom)
web/pages/deep_dive.py — Deep Dive page: per-analyst / per-product operational analytics with multi-select filter bar (analyst, product, time range). KPI stat cards (tickets closed, avg resolution days, avg complexity, frustrated count/%, avg first response hours). Tickets by Severity bar chart, Action Mix donut (InHance action class distribution), Monthly Closure Trend line chart, Resolution Time Distribution histogram (< 1d / 1-7d / 7-30d / 30-90d / 90+d), Workload Heatmap (analyst × product with hover showing avg days/complexity/frustrated), and filterable AG Grid of matching tickets with CSV export. All charts and grid update dynamically when filters change (custom)
