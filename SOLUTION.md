...
```bash
cd ts_ticket_review_and_update
python3 -m web.app
```

The dashboard runs at http://localhost:8050 with debug mode on (auto-reloads on code changes). Requires `DATABASE_URL` to be set in the environment or `.env` file. Migrations are applied automatically on startup.

## Updating Dependencies

When the Dash debug toolbar shows an update is available (e.g. "Dash update available - v4.1.0"):

```bash
pip3 install --upgrade dash
```

Then restart the app (`python -m web.app`). Other dashboard packages can be upgraded similarly:

```bash
pip3 install --upgrade dash-mantine-components dash-ag-grid dash-iconify
```
...
outlook_integration/   — Self-contained Microsoft Graph API client for Outlook / Office 365 (portable — copy folder to any Python project). Auth: device-code flow (delegated) + client-credentials (app-only) via MSAL with persistent token cache. Mail: list, get, search (KQL), send, reply-all, list folders. Calendar: list, get, calendarView, create (with Teams meeting link), update, delete, list calendars. Config via .env in-folder (GRAPH_CLIENT_ID, GRAPH_TENANT_ID, GRAPH_CLIENT_SECRET). See outlook_integration/README.md for Azure AD setup and usage examples.
...
run_import_work_item_comments.py — Pull comment text for all non-PBI work items with comment_count > 0 into work_item_comments table; fetches from /work-items/{id}/comments (top 50), upserts on (work_item_id, comment_id); used as the comment source for DO alignment enrichment so the LLM reads from DB rather than making live ADO API calls per ticket
run_do_alignment.py — DO alignment enrichment: for each open ticket with a linked DO (do_number), fetches the DO's state history (from work_item_updates) and recent comments (from work_item_comments), builds a structured JSON context block, calls Matcha LLM with the do_alignment prompt, and persists results (aligned: Yes/No/Partial, mismatch_label, explanation) to ticket_do_alignment; hash-based skip (thread_hash + do_state + latest comment date) avoids re-scoring unchanged tickets; triggered post-sync from enrich_tickets() in post_sync.py and after any run_import_work_items.py run; supports --ticket-id, --ticket-number, --limit, --force flags
activity_cleaner.py    — Text-cleaning pipeline (HTML→text, boilerplate/signature removal)
matcha_client.py       — Matcha LLM API client; `call_matcha()` for single-shot pipeline prompts; `call_matcha_chat()` for multi-turn chat using the API's native `messages`/`chat_history`/`context` fields with assistant-content normalization
...
db.py                  — Postgres data-access layer (connection pool, migration runner, upsert helpers, enrichment persistence, get_sync_state watermark reader, watermark-safe upsert_sync_state with optional watermark_at override, get_excluded_ticket_numbers(stage) returns ticket_numbers excluded from a given scoring stage via ticket_exclusions table; fetch_open_ticket_numbers_missing_sentiment/complexity queries filter out excluded tickets at the DB layer)
...
run_sentiment.py       — Part 2: sentiment analysis via Matcha (DB persistence + hash-based skipping when DB available; filters ticket_exclusions.exclude_sentiment before scoring loop)
run_priority.py        — Part 3: AI priority scoring via Matcha (DB persistence + hash-based skipping when DB available; filters ticket_exclusions.exclude_priority after hash-skip)
run_complexity.py      — Part 4: complexity analysis via Matcha (DB persistence + hash-based skipping when DB available; filters ticket_exclusions.exclude_complexity after hash-skip)
...
migrations/047_saved_reports_page_scope.sql — Adds `page TEXT NOT NULL DEFAULT 'tickets'` to saved_reports; drops old unique-on-name index and replaces with unique-on-(name,page) so Tickets and Health Dashboards each maintain independent saved report sets
migrations/046_exclude_scores_from_analytics_view.sql — Replaces vw_ticket_analytics_core to LEFT JOIN ticket_exclusions and wrap sentiment/priority/complexity fields in CASE WHEN exclusion flag → NULL, so excluded tickets contribute NULL scores to all rollups and health calculations
migrations/045_ticket_exclusions.sql — Per-ticket opt-out table: `ticket_exclusions` with `exclude_priority`, `exclude_sentiment`, `exclude_complexity` boolean flags and a `reason` text field; prevents flagged tickets from being scored by the respective AI stage (migration only — apply via `psql` or the migration runner)
migrations/049_work_item_comments.sql — work_item_comments table (PK: work_item_id, comment_id); stores comment text, author, created/modified dates for DO work items synced via run_import_work_item_comments.py
migrations/050_do_alignment.sql — ticket_do_alignment append-only table + vw_latest_ticket_do_alignment view (DISTINCT ON ticket_id, scored_at DESC); stores LLM-assessed alignment result per ticket (aligned, mismatch_label, explanation)
migrations/051_do_alignment_in_views.sql — Rebuilds vw_ticket_analytics_core and vw_operational_open_tickets to LEFT JOIN vw_latest_ticket_do_alignment, surfacing do_aligned, do_mismatch_label, do_alignment_explanation on every ticket row
...
web/pages/tickets.py   — Ticket explorer: AG Grid with filters, sorting, row-click navigation to detail, saved filter reports, background sync via subprocess with live log panel and auto-dismiss; tabbed view with Open (default, excludes Closed), All Tickets, and user-saved report tabs; saved reports are tabs rather than chips, can be deleted from the page, and the current tab/filter state is preserved when navigating to ticket detail and back; saved report tabs are reorderable via chevron-left/chevron-right buttons that swap sort_order with the adjacent tab (migration 037); rowSelection uses string format "single" for compatibility; filter persistence via browser sessionStorage — AG Grid filterModel and active tab are saved to a session-scoped dcc.Store on every change and restored automatically when navigating back from ticket detail, so filters survive page transitions until the user clicks Clear Filters; sync-progress-panel hidden via display:none when empty to avoid blank gap in stack layout; DO Align column (next to DO Status) shows color-coded mismatch label for tickets with a linked DO — red for hard mismatches (open/DO closed, closed/DO active, scope mismatch), yellow for stalled, tooltip shows the LLM explanation; Flag column (🚩) shows review flag status, sourced from ticket_flags table via LEFT JOIN in get_ticket_list() (custom)
web/pages/ticket_detail.py — Ticket detail: metadata header, thread timeline (newest-first), score cards, wait profile chart, issue summary, refresh button (re-syncs single ticket from TeamSupport via run_ingest.py --ticket-id, then re-renders page); ticket number in the header links to the TeamSupport UI, and the back link preserves the originating Tickets tab/report context; Scores tab includes a "Scoring Exclusions" panel with checkboxes for exclude_priority, exclude_sentiment, exclude_complexity and an optional reason field — saving writes to ticket_exclusions and automatically rescores all three stages (force=True, so hash-skip is bypassed; exclusion filter in run_* prevents excluded stages from running) then rebuilds customer and product health rollups in a background thread; all interactive component IDs use pattern-matching dicts `{"type": ..., "index": ctx}` where ctx is "page" (full /ticket/{id} route) or "modal" (embedded in health drilldown) — this allows the same callbacks (Refresh, Share in Teams, Flag, Scoring Exclusions, auto-poll) to work identically in both contexts; `build_ticket_shell(ticket_id, back_href, *, ctx)` creates the outer shell (stores, interval, indicator, content div) for either context; `_build_detail(ticket_id, back_href, *, ctx)` builds the header+tabs content — back link is omitted when back_href=None (modal), Share in Teams, Flag, and Refresh are always present; `ticket_detail_layout` delegates to `build_ticket_shell(ctx="page")`; DO Alignment displayed in three spots: (1) metadata header badge (`_build_do_alignment_meta`) — color-coded badge (green/yellow/red) + inline explanation text for any non-aligned result; (2) DO tab — alert callout at the top of the tab with label and explanation when aligned != "Yes"; (3) ticket list grid DO Align column; "Ask Matcha" tab: multi-turn chat against Matcha using ticket metadata + last 25 thread messages as injected context (`context` field); context includes a standing analyst instruction block loaded from the `ticket_chat_system` prompt (prompt store, `prompts/ticket_chat_system.md`) telling Matcha to reason like a senior analyst, lead with a bold **Net:** summary line when meaningful misalignments are found (DO/ticket mismatch, grab-bag ticket, stalled conversation), and treat status/field accuracy as a reflective check rather than mandatory preamble; when a DO # is present, the last 5 DO comments are fetched live and included in the context (`=== RECENT DO COMMENTS ===`) so Matcha can reason about comment drift vs. field state; DO # and DO status are explicit metadata fields; conversation history stored in a `dcc.Store` and passed as `chat_history` on each turn via `call_matcha_chat()`; LLM call runs in a background thread (`_run_chat`) with a `dcc.Interval` polling loop identical to the rescore pattern; `restore_chat_after_refresh` skips re-rendering when the chat poll is still active to prevent a race condition where an auto-refresh rebuild overwrites an in-flight response; background thread (`_run_chat`) with a `dcc.Interval` polling loop identical to the rescore pattern; Clear button resets history; input box is at the top, messages rendered below newest-first as styled Paper bubbles (blue=user, violet=Matcha) with a "Thinking…" pending indicator while the LLM responds (custom)
...
web/pages/health.py    — Health dashboards: Customer and Product health AG Grid tables; Customer Health now shows one row per active TeamSupport customer, includes `Key Account`, computes default distress across all groups except Marketing/Sales, and supports multi-row "View Tickets" drill-down plus single-customer history modal with group selector, Matcha-backed Explain button, and persisted explanation history; View Tickets drill-down modal now includes a Top Issue Clusters panel (top 3 clusters by ticket count for the selected customer(s)) with an open-only/all-tickets toggle, showing both L1 cluster key and mechanism class per card; Customer Health grid columns use flex sizing with minWidth so all columns fill the available viewport width; column order places Distress and Band immediately after Customer (before Key Acct) for quick triage visibility; grid now surfaces all five distress sub-dimensions (Pressure, Aging, Friction, Concentration, Breadth) matching the Health History dialog breakdown, each with color-coded highlights when above threshold; Concentration and Breadth are temporarily hidden from the grid/charts and excluded from distress score calculation via `EXCLUDED_DIMENSIONS` in `rollups/customer_health.py` (contributions still computed but zeroed before aggregation — remove the dimension name from the set to re-enable); NoPS column shows the distress score with Professional Services tickets excluded (computed at query time from contributor rows, no schema change); Generate Plan button produces a greedy target-band simulation (tickets sorted by total_contribution, removed until score falls below threshold) plus a Matcha-generated narrative plan, persisted with as_of_date versioning identical to explanations; target band is user-selectable (at_risk / watch / healthy) via a modal Select; `/health/plans` sub-page (`plans_layout`) renders an AG Grid of all saved plans across all customers with row-click detail modal; Customer Health tab has full saved-reports functionality: Save Report (captures current AG Grid filterModel), Delete, reorder (chevron buttons), Clear Filters, tabs for All Customers + saved report tabs — reports stored in saved_reports table scoped to page='health'; View Tickets drilldown modal has Tickets and Ask Matcha tabs — Tickets tab contains the existing list-view (grid + clusters) and inline ticket-view; Ask Matcha tab is a customer-scoped multi-turn Matcha chat that injects a summary of all open tickets for the selected customer(s) as context (`_build_customer_context`); system instructions loaded from the `customer_chat_system` prompt (prompt store, `prompts/customer_chat_system.md`) tell Matcha to proactively flag urgent tickets, DO↔ticket mismatches, and overall customer health; each ticket line includes DO # and DO status when present; uses `call_matcha_chat()` via a background thread with poll interval (same pattern as ticket chat), renders messages newest-first, and clears history on modal close; health-drilldown-grid is excluded from the generic cellClicked-to-navigate handler in app.py; Glossary tab: reference guide for health scoring dimensions (Distress, Band, Pressure, Aging, Friction, Concentration, Breadth) with definitions sourced from glossary.py HEALTH_GLOSSARY (custom)
web/pages/operations.py — Operations dashboard: analyst activity metrics covering workload distribution, skill mix, and contribution patterns. Overview KPI cards (CS group): Avg Days to Close (current month + past 6 months weighted average), CS Backlog at Jan 1 vs Now (with delta badge), Actionable Tickets by Analyst table (Mantine table with clickable analyst names — html.A pattern-matching links — open modal with full ticket list; excludes Pending (Customer), Complete, Survey, Confirm Resolution statuses and non-CS groups) paired side-by-side with Unassigned Tickets by Product table (clickable product names open modal with unassigned tickets for that product), and Most Improved Customers table (top 5 customers whose open backlog decreased most over last 3 months). Analyst Scorecard AG Grid (tickets closed, avg days open, high sev %, technical %, scheduling %, high priority/frustrated counts). Monthly Closures by Analyst line chart (date-range selectable, severity filter, split toggle). Tickets Created by Month bar chart. (custom)
web/pages/key_accounts.py — Key Account Tickets page: AG Grid of all open tickets for key accounts (customer_attributes.key_acct = TRUE). Columns include ticket number (clickable), customer, health score (color-coded by band), health band, ticket name, status, severity, assignee, product, group, age, priority, DO # (linked to Azure DevOps), DO Status, DO Align (color-coded yes/no), mismatch label, frustrated flag, created date. Health scores sourced from customer_health_ticket_contributors (same table/filters as health page: score_formula_version='v1', excluded health groups) so values match the Customer Health page exactly. Floating filters enabled. CSV export. Registered under Health nav section at /health/key-accounts. (custom)
...
## Ticket Detail — DO Tab with Work Item Status and Comments

When a ticket has a DO # linked, the ticket detail page shows an additional **DO #<id>** tab (Azure icon) containing:

- Work item metadata: DO # (clickable link to Azure DevOps), State, Type, Assigned To, Iteration, Changed date
- Work item title
- Comments from Azure DevOps (fetched live via `get_comments()` API), displayed newest-first with author, date, and cleaned text (HTML stripped)

| File | Change |
|------|--------|
| `web/data.py` | Added `get_do_comments(work_item_id)` — live API fetch of work item comments; `get_work_item_detail(work_item_id)` — DB lookup of single work item |
| `web/pages/ticket_detail.py` | Added conditional DO tab to the tabs list with work item metadata, title, and comment cards |
| `web/assets/dashAgGridFunctions.js` | `DOLink` React cell renderer for clickable DO # / work item ID links in AG Grid columns |

## Summary Tab — Complexity Analysis Detail

The Summary tab on ticket detail now shows the full complexity analysis (already generated by the complexity enrichment pass during `--enrich-new`). No new LLM calls or pipeline steps needed — it reads directly from `vw_latest_ticket_complexity`. Displayed sections:

- **Complexity Summary** — LLM-generated narrative of the ticket's core complexity
- **Evidence** — specific observations from the thread supporting the complexity assessment
- **Noise Factors** — scheduling/coordination churn that inflated ticket duration without adding real complexity
- **Duration vs Complexity** — explanation of how elapsed time relates to actual engineering difficulty

These appear automatically for any ticket that has been through complexity scoring (which runs as part of the "Refresh from TeamSupport" enrichment flow).

| File | Change |
|------|--------|
| `web/data.py` | Added `get_ticket_complexity_detail(ticket_id)` — fetches complexity_summary, evidence, noise_factors, duration_vs_complexity_note |
| `web/pages/ticket_detail.py` | Renders complexity detail sections in the Summary tab after any existing issue summaries |

## Auto-Refresh on Ticket Detail Load

When a user navigates to a ticket detail page the app immediately spawns a background thread that:

1. Re-syncs the ticket from TeamSupport (`run_ingest.py sync --ticket-id <id> --verbose --enrich-new`)
2. Refreshes Azure DevOps work items (`run_import_work_items.py`)

A small floating indicator ("Syncing latest data…") appears in the bottom-right corner while the sync is running. Once both subprocesses finish, the page silently re-renders with the latest data and the indicator disappears. The manual "Refresh from TeamSupport" button also triggers the same background sync flow instead of blocking the UI.

| File | Change |
|------|--------|
| `web/pages/ticket_detail.py` | Added `_auto_refresh` state dict, `_auto_lock`, `_run_auto_refresh()` background thread, `dcc.Interval` polling callback, fixed-position floating indicator |

## Share in Teams

A "Share in Teams" button in the ticket detail header opens a new Microsoft Teams chat with a pre-filled message containing:

- Ticket number, title, status, priority
- Customer, assignee, age, product
- DO # and status (if linked)
- Complexity summary (if available from enrichment)

The Teams "To" field is left empty so the user gets full directory auto-complete when typing a name. Clicking the button also logs a `teams_share` event in the `ticket_events` table with a preview of the message content.

### Activity Tab

A new **Activity** tab on the ticket detail page shows all user-initiated events (e.g. Teams shares) in a timeline, clearly separate from the TeamSupport thread and DevOps comments. Each event card shows the event type, timestamp, and a preview of the message content.

| File | Change |
|------|--------|
| `migrations/044_ticket_events.sql` | Creates `ticket_events` table (id, ticket_id, event_type, detail JSONB, created_by, created_at) |
| `web/data.py` | Added `insert_ticket_event()` and `get_ticket_events()` functions |
| `web/pages/ticket_detail.py` | Added "Share in Teams" button, `_build_teams_message()` helper, clientside callback to open Teams deep link, server callback to log event, Activity tab rendering event cards |
